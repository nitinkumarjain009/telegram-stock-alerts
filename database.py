...
CREATE_ALERTS_TABLE = "CREATE TABLE IF NOT EXISTS alerts (id INTEGER PRIMARY KEY, row_number INTEGER, ticker TEXT, alert_level TEXT, last_close REAL);"
GET_MAX_ROW_NUMBER = "SELECT MAX(row_number) FROM alerts;"
INSERT_ALERT = "INSERT INTO alerts (row_number, ticker, alert_level, last_close) VALUES (?, ?, ?, ?);"
...
DELETE_ALERT = "DELETE FROM alerts WHERE row_number=?;"
UPDATE_ROW_NUMBERS = ""
...
def delete_alert_by_row_number(connection, row_number):
    with connection:
        connection.execute(DELETE_ALERT, (row_number,))
        connection.execute(UPDATE_ROW_NUMBERS)
